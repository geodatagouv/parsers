...
class Context {
    ...
    constructor(parser, type, params) {
        ...
        this.acceptableChildren = params.acceptableChildren;
    ...
    onOpenTag(name, attrs) {
        if (this.isAcceptableChild(name)) {
            ...
            this.childContext = new Context(this.parser, 'child', {
                element: name,
                parentContext: this
            });
        ...
        if (this.isAcceptableContentChild(name)) {
            this.parser.toggleTextCapture(false);
            this.childContext = new Context(this.parser, 'contentChild', {
                element: name,
                parentContext: this
            });
            this.parser.pushContext(this.childContext);
        ...
        if (this.isAcceptablePropertyType(name)) {
            this.parser.toggleTextCapture(false);
            this.childContext = new Context(this.parser, 'propertyType', {
                element: name,
                parentContext: this
            });
            this.parser.pushContext(this.childContext);
    ...
    close() {
        if (this.parser.capturingText && ! this.internalValue) {
            this.internalValue = this.parser.textBuffer;
            this.parser.toggleTextCapture(false);
        }
