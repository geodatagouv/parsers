...
function parseDate(textValue) {
  if (textValue.length === 19) return new Date(textValue + 'Z')
...
class Context extends EventEmitter {
  ...
  onOpenTag(elementName, attributes) {
    ...
    this.parser.pushContext({
      ...
      onClose: value => {
        if (!isAcceptableValue(value)) return
...
class Parser extends Writable {
  ...
  constructor() {
    ...
    this.parser = sax.parser(true, { trim: true, strictEntities: true })
    this.parser.onopentag = e => this.onOpenTag(e.name, e.attributes)
    this.parser.onclosetag = name => this.onCloseTag(name)
    this.parser.ontext = text => this.onText(text)
    this.parser.onerror = err => this.emit('error', err)
    this.parser.onend = () => this.emit('end')
